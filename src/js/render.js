...
function renderContent(data) {
  const fallbackVideoItem = {
    title: 'Prosedur Facial Treatment',
    src: 'https://res.cloudinary.com/dwarsapox/video/upload/q_auto,f_auto,w_720/v1776465403/4264880-uhd_3840_2160_30fps_f3bhai.mp4',
    thumbnail: '/assets/images/service_aging.png',
    meta: 'Edukasi proses treatment langsung di klinik dengan prosedur higienis.'
  };

  const ensureVideoGridFallback = (videoGridEl, whatsappLink) => {
    if (!videoGridEl || videoGridEl.querySelector('.video-card')) return;
    videoGridEl.classList.add('video-grid--single');
    videoGridEl.innerHTML = `
      <article class="video-card video-card--featured" tabindex="0" aria-label="Video edukasi ${fallbackVideoItem.title}">
        <video poster="${getAssetUrl(fallbackVideoItem.thumbnail, 'tr=w-500')}" preload="metadata" muted loop playsinline autoplay controls>
          <source src="${getAssetUrl(fallbackVideoItem.src)}" type="video/mp4">
        </video>
        <div class="video-play-overlay">
          <button type="button" class="play-icon-circle video-play-toggle" aria-label="Putar video edukasi ${fallbackVideoItem.title}">
        ...
        <div class="video-info-bottom">
          <h3 class="video-card-title">${fallbackVideoItem.title}</h3>
          <p class="video-card-meta">${fallbackVideoItem.meta}</p>
          <a href="${whatsappLink}" class="video-cta-btn" target="_blank">
            Konsultasi Sekarang
  ...
  if (data.hero && heroContent && heroBg) {
    if (data.hero.backgroundVideo) {
      heroBg.innerHTML = `
        <video autoplay muted loop playsinline class="hero-video" fetchpriority="high">
  ...
  if (data.sections.promo && promoGrid) {
    ...
    promoGrid.innerHTML = data.sections.promo.items.map((item, index) => {
      if (isFlyer) {
        return `
          <a href="${item.link}" class="promo-card promo-flyer animate-on-scroll" target="_blank" style="--delay: ${index * 0.1}s" aria-label="Ambil Promo: ${item.title}">
            <div class="flyer-image">
              <img src="${getAssetUrl(item.image)}" alt="Flyer Promo ${item.title}" loading="lazy" width="360" height="509">
            </div>
      ...
      return `
        <a href="${item.link}" class="promo-card animate-on-scroll" target="_blank" style="--delay: ${index * 0.1}s" aria-label="Ambil Promo: ${item.title}">
          ...
          <div class="promo-image">
            <img src="${getAssetUrl(item.image)}" alt="Ilustrasi ${item.title}" loading="lazy" width="200" height="250">
          </div>
  ...
  if (data.sections.services && servicesHeader && servicesGrid) {
    ...
    servicesGrid.innerHTML = itemsToRender.map(item => `
      <div class="service-card animate-on-scroll" id="service-${item.id}">
        <div class="service-img">
          ...
          ${item.video ? `<video src="${getAssetUrl(item.video)}" preload="metadata" muted loop playsinline></video>` : ''}
        ...
        <div class="service-content">
          ...
          <a href="${item.link}" class="service-link" target="_blank" aria-label="Tanya detail mengenai layanan ${item.title}">
            Selengkapnya
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M5 12h14M12 5l7 7-7 7" />
            </svg>
    ...
    // Add Video Interactivity (Ergonomics Update)
    ...
    if (isMobile) {
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          const video = entry.target.querySelector('.service-img video');
          if (video) {
  ...
  if (data.sections.videos && videoHeader && videoGrid) {
    const rawVideoItems = Array.isArray(data.sections.videos.items) ? data.sections.videos.items : [];
    const videoItems = rawVideoItems.length ? rawVideoItems : [fallbackVideoItem];
    const isSingleVideo = videoItems.length === 1;
    ...
    videoGrid.classList.toggle('video-grid--single', isSingleVideo);
    videoGrid.innerHTML = videoItems.map(item => `
      <article class="video-card ${isSingleVideo ? 'video-card--featured' : ''}" tabindex="0" aria-label="Video edukasi ${item.title}">
        <video poster="${getAssetUrl(item.thumbnail, 'tr=w-500')}" preload="metadata" muted loop playsinline autoplay controls>
          <source src="${getAssetUrl(item.src)}" type="video/mp4">
        </video>
        <div class="video-play-overlay">
          <button type="button" class="play-icon-circle video-play-toggle" aria-label="Putar video edukasi ${item.title}">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
              <path d="M8 5v14l11-7z"/>
            </svg>
          </button>
        ...
        <div class="video-info-bottom">
          ...
          <p class="video-card-meta">${item.meta || fallbackVideoItem.meta}</p>
          <a href="${data.hero.whatsappLink}" class="video-cta-btn" target="_blank" aria-label="Konsultasi mengenai treatment di video ${item.title}">
            Konsultasi Sekarang
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M5 12h14M12 5l7 7-7 7" />
            </svg>
    ...
    // Add video interactivity with explicit user controls.
    const videoCards = videoGrid.querySelectorAll('.video-card');
    const isMobileVid = window.matchMedia("(max-width: 992px)").matches;
    const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    videoCards.forEach(card => {
      const video = card.querySelector('video');
      const toggleBtn = card.querySelector('.video-play-toggle');
      if (!video || !toggleBtn) return;

      const setPlayingState = (isPlaying) => {
        card.classList.toggle('is-playing', isPlaying);
        toggleBtn.setAttribute('aria-label', `${isPlaying ? 'Jeda' : 'Putar'} video edukasi`);
      };
    ...
    if (!prefersReducedMotion && isMobileVid) {
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
    ...
    ensureVideoGridFallback(videoGrid, data.hero.whatsappLink);
  }
  ...
  const waNumber = data.hero.whatsappLink.replace('https://wa.me/', '');
  
  const getContextualLink = (message) => {
  ...
  // 3.5 Video
  if (videoGrid) {
    const videoCtaButtons = videoGrid.querySelectorAll('.video-cta-btn');
    const ctaVideoItems = Array.isArray(data.sections?.videos?.items) && data.sections.videos.items.length
      ? data.sections.videos.items
      : [{ title: 'Video Treatment' }];
    videoCtaButtons.forEach((btn, i) => {
      const videoTitle = ctaVideoItems[i]?.title || 'Video Treatment';
      btn.href = getContextualLink(`Halo Deoksi Clinic, saya baru saja menonton video ${videoTitle} dan tertarik untuk konsultasi.`);
    });

    ensureVideoGridFallback(videoGrid, getContextualLink('Halo Deoksi Clinic, saya ingin konsultasi mengenai treatment.'));
  }



  document.querySelectorAll('.wa-link').forEach(link => {
    link.href = data.hero.whatsappLink; // Fallback for general links
  });
}
